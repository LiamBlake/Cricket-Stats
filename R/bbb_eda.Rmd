...
```{r}
follow_ons_recipe <- recipe(follow_on ~ ., data = follow_ons)
follow_ons_model <- logistic_reg() %>%
set_engine("glm")

follow_ons_wf <- workflow() %>% 
  add_recipe(follow_ons_recipe) %>% 
  add_model(follow_ons_model)

follow_ons_wf %>% fit(follow_ons) %>% pull_workflow_fit()
```
