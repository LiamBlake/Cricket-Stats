...
# Preprocessing
```{r}
wkt_recp <- recipe(is_wkt ~ ., data = bbb_train) %>%
...
```{r}
wf_logit <- workflow() %>% add_recipe(recipe = wkt_recp)
wf_logit <- wf_logit %>% add_model(logistic_reg(penalty = 0) %>% set_engine("glmnet"))

wf_logit <- wf_logit %>% fit(data = bbb_train)
fit_logit_all_noint <- wf_logit %>% pull_workflow_fit()
...
```{r}
test_logit_all_notint <- bbb_test %>% add_column(
  fit_logit_all_noint %>% predict(new_data = wkt_recp %>% prep(bbb_test), type = "prob")
)
```
